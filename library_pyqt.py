...
from PyQt5.QtCore import pyqtSignal, QObject, QTime, QTimer
...
class FinanceWindow(QMainWindow):
    def __init__(self):
        ...
        self.index_list_world = slib.StockList('index')
        self.index_list_kor = slib.StockList('ind_kor')
        self.korea_list = slib.StockList('Korea')
        ...
        btn_world_index.clicked.connect(self.update_world_index)
        ...
        self.status_txt = 'Stock price update UI\n'
        self.edit_status.setPlainText(self.status_txt)
    ...
    def status_update(self, inputstr):
        self.status_txt = self.status_txt + inputstr + '\n'
        self.edit_status.setPlainText(self.status_txt)
        self.edit_status.verticalScrollBar().setValue(self.edit_status.verticalScrollBar().maximum())
    ...
    def hold_awhile(self):
        if self.timeroff == 2:
            ...
            holdtime = 70 - deltatime.seconds + 1
            if holdtime < 0:
                holdtime = 0
            self.status_update('Hold on for ' + str(holdtime) + ' sec to avoid the request limitation')
            self.timer.setInterval(holdtime * 1000)
            self.timer.start()
            return
    ...
    def update_world_index(self):
        self.index_list_world.update_list_yf()
        return
