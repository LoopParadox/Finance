...
class StockData(QObject):
    ...
    def update_history_yh(self, **kwargs):
        if self.yahoo:
            if 'start' in kwargs:
                start = kwargs['start']
                ...
                self.log('Enter some input arguments at "start" or "period"')
                return
            try:
                self.history = self.history.append(update)
                return
            except:
                self.log('Got some errors')
                return
        else:
            return
    ...
    def sort_drop_dup(self):
        ...
        if not self.history.index.is_unique:
            ...
            for idx in unindex:
                ...
                self.history = self.history.iloc[0:sliceidx.start].append(remdup).append(
                    self.history.iloc[sliceidx.stop:])
